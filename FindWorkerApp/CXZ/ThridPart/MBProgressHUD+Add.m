       ...
       // hud = [MBProgressHUD showHUDAddedTo:view Frame:[self getKeyWindowFrame] animated:YES];
        ...
        if([view isKindOfClass:[UITableView class]]){
            ...
            if (tableview.contentSize.height>SCREEN_HEIGHT) {
                hud.yOffset=(tableview.contentSize.height-SCREEN_HEIGHT);
            }
...
+ (void)show:(NSString *)mes after:(NSInteger)num toView:(UIView *)view {
    // 快速显示一个提示信息
    MBProgressHUD *hud = [MBProgressHUD showHUDAddedTo:view animated:YES];
    // HUD.labelText = @"Only 10 notes for strangers. You can send 0 note now.";
    //自动换行
