...
@property (nonatomic, assign) CGFloat jiancai;//剪裁比例
...
- (id)initWithImage:(UIImage *)originalImage limitScaleRatio:(NSInteger)limitRatio {
    ...
    if (self) {
        ...
        self.originalImage = [CXZImageUtil fixOrientation:originalImage];
    }
    return self;
}
...
- (void)viewDidLoad
{
    [super viewDidLoad];
    [self setupBack];
    UIImage * image = [CXZImageUtil imageWithColor:[UIColor colorWithRed:0.00 green:0.00 blue:0.00 alpha:1.00] Size:CGSizeMake(SCREEN_WIDTH, 64)];
...
- (void)viewWillDisappear:(BOOL)animated {
    [self.navigationController.toolbar setHidden:_toolBarStatus];
}
...
- (void)initControlBtn {
    [self.navigationController.toolbar setBackgroundImage:[CXZImageUtil imageWithColor:TINTCOLOR Size:self.navigationController.toolbar.frame.size] forToolbarPosition:UIBarPositionBottom barMetrics:UIBarMetricsDefault];
