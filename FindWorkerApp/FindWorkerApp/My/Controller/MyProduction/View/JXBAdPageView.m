...
- (void)initUI {
    ...
    _imgNumLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, self.frame.size.height-10, self.frame.size.width, 10)];
    _imgNumLabel.textAlignment = NSTextAlignmentRight;
    _imgNumLabel.textColor = [UIColor whiteColor];
    _imgNumLabel.font = [UIFont systemFontOfSize:12];
    
    [self addSubview:_imgNumLabel];
}
...
- (void)startAdsWithBlock:(NSArray*)imageArray block:(JXBAdPageCallback)block {
   _imgNumLabel.text = [NSString stringWithFormat:@"1/%li",imageArray.count];
...
- (void)reloadImages {
    ...
    _imgNumLabel.text = [NSString stringWithFormat:@"%i/%li",(_indexShow+1),self.imgCount];
}
