...
@interface ApprovalViewController ()<UITableViewDelegate,UITableViewDataSource,UIScrollViewDelegate,SponsorTableViewCellDelegate>
{
    NSInteger page;
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(loadNewData) name:@"UPLOAD_NEW_DATA" object:nil];
}
...
-(void)changeCompany
{
    if (self.companyArr.count>0) {
        ...
        JXPopoverAction *action = [JXPopoverAction actionWithTitle:@"全部" handler:^(JXPopoverAction *action) {
            self.companyID = @"";
            [self.firstArray removeAllObjects];
...
-(void)clickSearch
{
    ...
    searchVC.searchType = searchApprovalType;
    searchVC.companyID = self.companyID;
...
-(void)clickSeeDetail:(NSInteger)index
{
    ...
    NSDictionary *paramDict = @{@"uid":[[NSUserDefaults standardUserDefaults] objectForKey:@"user_id"],
                                ...
                                @"company_id":self.companyID};
    [[NetworkSingletion sharedManager]deleteMyApproval:paramDict onSucceed:^(NSDictionary *dict) {
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (tableView == self.thirdTableview) {
        ...
        if (self.thirdArray.count >0) {
            [cell setSponsorCellWith:self.thirdArray[indexPath.row]];
        }
        return cell;
    }
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (tableView == self.firstTableview) {
        ...
        reviewVC.is_aready_approval = YES;
        reviewVC.is_cancel = YES;
        
        payVC.is_aready_approval = YES;
         payVC.is_cancel = YES;
        
        purchaseVC.is_aready_approval = YES;
         purchaseVC.is_cancel = YES;
        
        stampVC.is_aready_approval = YES;
        stampVC.is_cancel = YES;

        fileVC.is_aready_approval = YES;
        fileVC.is_cancel = YES;
        
        companyVC.is_aready_approval = YES;
        companyVC.is_cancel = YES;
    ...
    if (listModel) {
        if (listModel.type == 1 || listModel.type == 2) {
            ...
            reviewVC.is_sepcial = self.isSpecial;
