...
#define kDefaultCalendarBasicColor [UIColor colorWithRed:88/ 255.0 green:196/ 255.0 blue:196/ 255.0 alpha:1.0]
...
- (instancetype)initWithFrameOrigin:(CGPoint)origin width:(CGFloat)width {
    ...
    // calendar 头部栏高度
    CGFloat calendarHeaderHeight = 1 * weekLineHight;
...
- (void)setDidCurrentMounthHandler:(DidCurrentMounthHandler)didCurrentMounthHandler{
    _didCurrentMounthHandler = didCurrentMounthHandler;
    if (_calendarScrollView != nil) {
        _calendarScrollView.didCurrentMounthHandler  = _didCurrentMounthHandler; // 传递 block
    }
}
...
{
    [_calendarScrollView reloadCurrentMonthSignDay:calendarArray];
}
