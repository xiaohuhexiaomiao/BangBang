...
//    NSLog(@"haha %lf",self.bounds.size.width);
...
-(void)reloadCurrentMonthSignDay:(NSArray *)calendarArray
{
    self.signArray = calendarArray;
     [_collectionViewM reloadData];
}
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (collectionView == _collectionViewL) {
        ...
        if (indexPath.row >= firstWeekday && indexPath.row < firstWeekday + totalDays) {
            ...
            NSString *dayStr =[NSString stringWithFormat:@"%li年%li月%li日",monthInfo.year,monthInfo.month,indexPath.row - firstWeekday + 1];

            if (self.signArray.count > 0 && [self.signArray containsObject:dayStr]) {
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (month > [[NSDate date] dateMonth]) {
         return;
    }
    if ( month == [[NSDate date] dateMonth] && day >[[NSDate date] dateDay]) {
        return;
    }
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    if (scrollView.contentOffset.x < self.bounds.size.width) {
        ...
        if (self.didCurrentMounthHandler != nil) {
            self.didCurrentMounthHandler(currentMothInfo.year, currentMothInfo.month); // 执行回调
        }
